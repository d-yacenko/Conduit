...
public class DataHelper {
	...
	public static final String INSERT_T0_1 = "INSERT INTO "
			+ DBTables.T0_TABLE_NAME + " (" + DBTables.T0_COLUMN_ID + "," + DBTables.T0_COLUMN_PASSWORD + ","
			+ DBTables.T0_COLUMN_DB_PASSWORD + ","
			+ DBTables.T0_COLUMN_REPLICA_ID + ") values (?,?,?,?); ";
	...
	public static final String INSERT_T1_1 = "INSERT INTO "
			+ DBTables.T1_TABLE_NAME + " (" + DBTables.T1_COLUMN_ID + "," + DBTables.T1_COLUMN_NAME + ","
			+ DBTables.T1_COLUMN_LASTNAME + "," + DBTables.T1_COLUMN_SURNAME
			+ "," + DBTables.T1_COLUMN_EMAIL + "," + DBTables.T1_COLUMN_PHONE
			+ "," + DBTables.T1_COLUMN_PARENTPHONE + ","
			+ DBTables.T1_COLUMN_BIRTHDAY + "," + DBTables.T1_COLUMN_NUMCLASS
	...
	public static final String INSERT_T2_1 = "INSERT INTO "
			+ DBTables.T2_TABLE_NAME + " (" + DBTables.T2_COLUMN_ID + ","+ DBTables.T2_COLUMN_NAME + ","
			+ DBTables.T2_COLUMN_CITY + "," + DBTables.T2_COLUMN_TEACHER + ","
			+ DBTables.T2_COLUMN_ADDITIONAL + ","
			+ DBTables.T2_COLUMN_TIMETABLE + ") values (?,?,?,?,?,?); ";
	...
	public static final String INSERT_T3_1 = "INSERT INTO "
			+ DBTables.T3_TABLE_NAME + " (" + DBTables.T3_COLUMN_ID	+ "," + DBTables.T3_COLUMN_TEACH_GROUP_ID
			+ "," + DBTables.T3_COLUMN_STUDENT_ID + ") values (?,?,?); ";
	...
	public static final String INSERT_T4_1 = "INSERT INTO "
			+ DBTables.T4_TABLE_NAME + " (" + DBTables.T4_COLUMN_ID + "," + DBTables.T4_COLUMN_SUBJECT + ","
			+ DBTables.T4_COLUMN_DATE + "," + DBTables.T4_COLUMN_STUDENT_ID
			+ "," + DBTables.T4_COLUMN_GROUP_ID + ","
			+ DBTables.T4_COLUMN_VALUE + ") values (?,?,?,?,?,?); ";
	...
	class _Student extends Student{
		public _Student(Cursor cursor) {
			super(cursor.getInt(0),decode(cursor.getString(1)),
					decode(cursor.getString(2)),
					decode(cursor.getString(3)),
					decode(cursor.getString(4)),
					decode(cursor.getString(5)),
					decode(cursor.getString(6)),
					cursor.getLong(7)==0?null:new Date(cursor.getLong(7)),
					decode(cursor.getString(8)),
					Boolean.valueOf(decode(cursor.getString(9))),
					decode(cursor.getBlob(10)));
		}
	}
	class _Group extends Group {
		public _Group(Cursor cursor) {
			super(cursor.getInt(0),
					decode(cursor.getString(1)),
					decode(cursor.getString(2)),
					decode(cursor.getString(3)),
					decode(cursor.getString(4)),
					decode(cursor.getString(5)));
	...
	public List<Student> selectAll_student() {
		...
		if (cursor.moveToFirst())
			do { list.add(new _Student(cursor));
	...
	public List<Student> replicaStudent(){
		...
		Cursor cursor = db.query(DBTables.T1_TABLE_NAME, new String[] {
				...
				DBTables.T1_COLUMN_PHOTO }, null, null, null, null, null);
		if (cursor.moveToFirst())
			do { list.add(new Student(cursor.getInt(0),cursor.getString(1),
					cursor.getString(2),
					cursor.getString(3),
					cursor.getString(4),
					cursor.getString(5),
	...
	public List<Group> replicaGroup() {
		...
		if (cursor.moveToFirst())
			do {
				list.add(new Group(cursor.getInt(0),
						cursor.getString(1),
						cursor.getString(2),
						cursor.getString(3),
						cursor.getString(4),
	...
	public List<Student> selectAll_studentForGroup(Group group) {
		List<Student> list = new ArrayList<Student>();
		Cursor cursor = db.query(DBTables.T1_TABLE_NAME
				+ " as ST inner join " + DBTables.T3_TABLE_NAME
				+ " as GR on ST._id = GR.student_id", new String[] {
				"ST." + DBTables.T1_COLUMN_ID, "ST." + DBTables.T1_COLUMN_NAME,
				...
				"ST." + DBTables.T1_COLUMN_PHOTO }, "GR.teach_group_id = ?",
	...
	public List<GroupList> replicaGroupList() {
		...
		if (cursor.moveToFirst())
			do {
				list.add(new GroupList(cursor.getInt(0),
		...
		if(lessons.get(i).subject!=null){
			cursor = this.db.query(
					DBTables.T4_TABLE_NAME + " as AC inner join "
							+ DBTables.T1_TABLE_NAME + " as ST on ST._id = AC."
					...
					new String[] { lessons.get(i).subject,
							...
							String.valueOf(group.get_id()) }, null, null, null);
		}else {
			cursor = this.db.query(
					DBTables.T4_TABLE_NAME + " as AC inner join "
							+ DBTables.T1_TABLE_NAME + " as ST on ST._id = AC."
					...
					new String[] { "ST." + DBTables.T1_COLUMN_ID,
							...
							"ST." + DBTables.T1_COLUMN_PHOTO,
							"AC." + DBTables.T4_COLUMN_VALUE },
					"AC." + DBTables.T4_COLUMN_SUBJECT + " is null and AC."
							+ DBTables.T4_COLUMN_DATE + "=? and AC."
							+ DBTables.T4_COLUMN_GROUP_ID + "=? ",
			...
			if (cursor.moveToFirst())
				do {
					account.list.add(new RowAccountRecordModel(new _Student(cursor), group, decode(cursor
							.getString(11))));
				} while (cursor.moveToNext());
	...
	public List<Account> replicaAccount() {
		...
		Cursor cursor = db.query(DBTables.T4_TABLE_NAME, new String[] {
				...
				DBTables.T4_COLUMN_DATE, 
				DBTables.T4_COLUMN_STUDENT_ID, 
				DBTables.T4_COLUMN_GROUP_ID, 
				DBTables.T4_COLUMN_VALUE},
				null, null, null, null, null);
		if (cursor.moveToFirst())
			do {
				list.add(new Account(cursor.getInt(0),cursor.getString(1),cursor.getLong(2),
	...
	public void replicaASecurity(List<ASecurity> list) {
		...
		for(ASecurity as:list){
			...
			if(as.getReplicaId()!=null)stmt.bindString(4, as.getReplicaId());
			stmt.executeInsert();
		}
		db.execSQL("PRAGMA foreign_keys=ON;");
	}
